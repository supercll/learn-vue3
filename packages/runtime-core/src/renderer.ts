...
export function createRenderer(options) {
  ...
  function mountComponent(vnode, container, anchor, parent) {
    ...
    // 组件的优点是什么？  复用、逻辑拆分、方便维护、vue组件级更新
  }
  function patchKeyedChildren(c1, c2, el) {
    ...
    if (index > e1) {
      ...
      for (let i = s1; i <= e1; i++) {
        ...
        if (newIndex == null) {
          unmount(oldVNode, parent) // 新的里面找不到了，说明要移除掉
        } else {
  ...
  function patchChildren(n1, n2, el, parent) {
    ...
    if (shapeFlag & ShapeFlags.TEXT_CHILDREN) {
      if (prevShapeFlag & ShapeFlags.ARRAY_CHILDREN) {
        unmountChildren(c1, parent) //  文本	 数组	（删除老儿子，设置文本内容）
      }
      if (c1 !== c2) {
  ...
  function setupRenderEffect(instance, container, anchor) {
    const componentUpdate = () => {
      ...
      if (!instance.isMounted) {
        ...
        patch(null, subTree, container, anchor, instance) // 子组件得分父亲就是当前的实例 ， 就构建好了父子关系
  ...
  function processComponent(n1, n2, container, anchor, parent) {
    if (n1 == null) {
      if (n2.shapeFlag & ShapeFlags.COMPONENT_KEPT_ALIVE) {
        ...
        // 当第一次component1 卸载的时候 我需要将dom元素 移动到内存中，下次在渲染我们再将他拿回来
  ...
  function unmount(n1, parent) {
    ...
    if (n1.type == Fragment) {
      ...
      return unmount(component.subTree, parent) // 组件要卸载的是subTree 而不是自己
    }
